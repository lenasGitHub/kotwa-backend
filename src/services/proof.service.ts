...
export class ProofService {
  ...
  static async uploadProof(userId: string, habitId: string, imageUrl: string) {
    ...
    const lastProof = await prisma.habitProof.findFirst({
      where: {
        userId,
        habitId,
        status: 'APPROVED',
    ...
    // Create proof record
    const proof = await prisma.habitProof.create({
      data: {
        userId,
        habitId,
        imageUrl,
  ...
  static async voteOnProof(
    voterId: string,
    proofId: string,
    voteType: 'APPROVE' | 'REJECT',
  ) {
    const proof = await prisma.habitProof.findUnique({
      where: { id: proofId },
      include: {
        habit: {
          include: {
    ...
    const isJoined = proof.habit.joinedUsers.some((u: any) => u.id === voterId);
    if (!isJoined) {
      throw new Error('You must be part of this habit to vote');
    }

    // Check if already voted
    const existingVote = proof.votes.find((v: any) => v.voterId === voterId);
    if (existingVote) {
    ...
    const approvals = votes.filter((v: any) => v.voteType === 'APPROVE').length;
    const rejections = votes.filter((v: any) => v.voteType === 'REJECT').length;
    ...
    if (approvals >= 1 && newStatus === 'PENDING') {
      newStatus = 'APPROVED';
      await prisma.habitProof.update({
        where: { id: proofId },
  ...
  static async getPendingProofs(habitId: string, userId: string) {
  ...
  static async getMyProgress(habitId: string, userId: string) {
    const proofs = await prisma.habitProof.findMany({
      where: {
        habitId,
        userId,
    ...
    const approvedRounds = proofs.filter(
