...
class SocialService {
    ...
    static async createInvite(userId, habitId, phoneNumber) {
        ...
        const invite = await prisma_1.default.habitInvite.create({
            data: {
                code,
    ...
    static async acceptInvite(userId, code) {
        // Get the accepting user's phone number
        const acceptingUser = await prisma_1.default.user.findUnique({
            where: { id: userId },
            select: { phoneNumber: true },
        });
        const invite = await prisma_1.default.habitInvite.findUnique({
            where: { code },
            include: {
                habit: {
                    include: {
    ...
    static async syncContacts(userId, phoneNumbers) {
    ...
    static async getFriends(userId, habitId) {
        const friendships = await prisma_1.default.friendship.findMany({
            where: {
                userId,
                status: 'ACCEPTED',
